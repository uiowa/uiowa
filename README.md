...
### Common Tasks
The `drupal:sync:all-sites` command will generate local settings (by invoking `blt:init:settings`, or `bis` for short) files _only_ if they do not exist. If you want to re-generate all multisite local settings files, you can run `rm -f docroot/sites/*/settings/local.settings.php` beforehand.
