...
Drupal core requires the following specific command to update dev dependencies properly: `composer update drupal/core drupal/core-dev --with-dependencies`. You can run `composer update package/name` after that to update additional dependencies. The output from the Composer commands can be used as the long text for commit messages. Ideally, each package update would be one commit to the composer.lock file.

Certain scaffold files need to be manually updated. In particular, the `docroot/.htaccess` file needs to be manually updated since we have disabled drupal/core-compose-scaffold from downloading it. Check the core release notes to see if there were any changes. Different updates may require difference procedures.
