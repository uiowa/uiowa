...
## Best Practices
Config under `config/` is generated by Drupal's export rather than written by hand. Make the change in the site and export it. Hand-editing is the exception and needs a reason, since the next export normalizes the file and produces a diff against the edit.
