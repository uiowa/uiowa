...
class cevalidationsrOverviewController extends ControllerBase {
  ...
  public function showOverview() {
    $build = [];
    [$response, $json] = $this->pingEndpoint($build);
    // If response data was built and returned, display it with a sample of the.
    // Objects returned.
    if (isset($response)) {
      $build['response'] = ['#theme' => 'item_list', '#title' => t('Response: @r', ['@r' => $response->getReasonPhrase()]), '#items' => ['code' => t('Code: @c', ['@c' => $response->getStatusCode()])]];
    }
    if (isset($json)) {
      $build['response_data'] = ['#theme' => 'item_list', '#title' => t('Response Data:'), '#items' => ['response-type' => t('Response Type: @t', ['@t' => $json->response_type]), 'total-count' => t('Total Count: @c', ['@c' => $json->pagination->total_count])]];
  ...
  protected function pingEndpoint(&$build) {
    ...
    try {
      ...
      $build['server_error'] = ['#theme' => 'item_list', '#title' => t('Server Exception: @r', ['@r' => $e->getResponse()->getReasonPhrase()]), '#items' => ['url' => t('URL: @u', ['@u' => $e->getRequest()->getUri()]), 'code' => t('Code: @c', ['@c' => $e->getResponse()->getStatusCode()])]];
      $build['exception'] = ['#markup' => $e->getMessage()];
      $logger = \Drupal::logger('cevalidationsr');
    }
    catch (ClientException $e) {
      // Handle client-side error (e.g., authorization failures)
      $build['client_error'] = ['#theme' => 'item_list', '#title' => t('Client Exception: @r', ['@r' => $e->getResponse()->getReasonPhrase()]), '#items' => ['url' => t('URL: @u', ['@u' => $e->getRequest()->getUri()]), 'code' => t('Code: @c', ['@c' => $e->getResponse()->getStatusCode()])]];
      $build['exception'] = ['#markup' => $e->getMessage()];
      $logger = \Drupal::logger('cevalidationsr');
    }
    catch (\Exception $e) {
      // Handle general PHP exemptions.
      $build['php_error'] = ['#theme' => 'item_list', '#title' => t('PHP Exception'), '#items' => ['code' => t('Code: @c', ['@c' => $e->getCode()])]];
      $build['exception'] = ['#markup' => $e->getMessage()];
      $logger = \Drupal::logger('cevalidationsr');
    }
  ...
  protected function displayDataSample($json, &$build) {
    if (isset($json->response_data[0])) {
      ...
      $build['cevalidationsr_sample'] = ['#prefix' => '<pre>', '#markup' => print_r($cevalidationsr_data, TRUE), '#suffix' => '</pre>'];
    }
  }
