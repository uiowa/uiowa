...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let layoutContentMO = new window.MutationObserver(function (e) {
      ...
      for (let i = 0; i < e.length; i++) {
        if (e[i].removedNodes[0] && e[i].removedNodes[0].id === 'layout-builder') {
          // Delete the old markup and regenerate the map.
          reGenerateMap();
        }
      }
    });
  ...
  // This is to generate the map on first DOM load.
  generateMap();
});

// This function looks for the map container, and if it is there, recreates it and generates a new map.
function reGenerateMap() {
  // Find the map container.
  let mapHolder = document.getElementById("admissions-counselors-map");
  // If it exists...
  if(mapHolder) {
    // create a copy of the needed map container...
    // ('<div id="admissions-counselors-map">&nbsp;</div>')
    let newNode = document.createElement('div');
    newNode.id = "admissions-counselors-map";
    newNode.innerHTML = "&nbsp";

    // Replace the old container.
    mapHolder.parentNode.replaceChild(newNode, mapHolder);
