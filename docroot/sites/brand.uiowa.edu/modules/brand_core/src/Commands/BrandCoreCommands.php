  ...
  public function lockupDigest($options = ['msg' => FALSE]) {
    ...
    if (!empty($view)) {
      ...
      if ($result['result'] !== TRUE) {
        \Drupal::logger('brand_core')->error($this->t('Lockup Review Digest Not Sent'));
        $this->output()->writeln('Lockup Review Digest Not Sent');
      }
      else {
        \Drupal::logger('brand_core')->notice($this->t('Lockup Review Digest Sent'));
        $this->output()->writeln('Lockup Review Digest Sent');
      }
    }
    else {
      \Drupal::logger('brand_core')->notice($this->t('Lockup Review Digest - No items to review'));
