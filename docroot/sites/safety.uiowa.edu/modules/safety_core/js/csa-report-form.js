(function (Drupal, once) {
  ...
  Drupal.behaviors.csaReportFormClear = {
    attach: function (context, settings) {
      ...
      const dateTypeSelects = once('date-clear', '[name="occurrence_date_type"]', context);
      dateTypeSelects.forEach(function(select) {
        select.addEventListener('change', function() {
          if (this.value === 'unknown') {
            // Clear all date-related fields.
            const dateOffenseOccured = document.querySelector('[name="date_offense_occured"]');
            ...
            const timeTypeSelect = document.querySelector('[name="occurrence_time_type"]');
            if (timeTypeSelect) {
      ...
      const timeTypeSelects = once('time-clear', '[name="occurrence_time_type"]', context);
      timeTypeSelects.forEach(function(select) {
        select.addEventListener('change', function() {
          if (this.value === 'unknown') {
            // Clear all time-related fields.
            const exactTimeOccured = document.querySelector('[name="exact_time_occured"]');
